...
const createOrder = async () => {
  ...
  try {
    ...
    showError(e, 'Erro ao criar pedido');
  } finally {
...
const openEditModal = () => {
  if (selectedOrder.value) {
    if (!currentUser.value?.is_admin && selectedOrder.value.user_id !== currentUser.value?.id) {
      showError(
...
const updateOrder = async () => {
  ...
  try {
    ...
    showError(e, 'Erro ao atualizar pedido');
  } finally {
      ...
      <div class="flex items-center justify-center min-h-screen px-4">
        <div class="relative bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <div class="px-6 py-4" v-if="selectedOrder">
            <div v-if="!showEditModal">
              ...
              <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700">Data de Ida</label>
                  <p class="mt-1 text-sm text-gray-900">{{ formatDate(selectedOrder.start_date) }}</p>
                </div>
                <div>
                  <label class="block text-sm font-medium text-gray-700">Data de Volta</label>
                  <p class="mt-1 text-sm text-gray-900">{{ formatDate(selectedOrder.end_date) }}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p class="mt-1 text-sm text-gray-900">{{ formatDate(selectedOrder.updated_at) }}</p>
                </div>
              </div>
            </div>
          ...
          <div class="flex justify-end px-6 py-4 space-x-3 border-t border-gray-200">
            ...
            <button
              v-if="!showEditModal && currentUser?.is_admin"
              @click="openEditModal"
